...
func TestAssertLess(t *testing.T) {
	assert := test.NewAssert(t)
	err := test.IsSolved(&assertLessCircuit{}, &assertLessCircuit{A: NewU32(1), B: NewU32(2), Expected: frontend.Variable(1)}, ecc.BN254.ScalarField())
	assert.NoError(err)

	err = test.IsSolved(&assertLessCircuit{}, &assertLessCircuit{A: NewU32(2), B: NewU32(1), Expected: frontend.Variable(0)}, ecc.BN254.ScalarField())
	assert.Error(err)

	err = test.IsSolved(&assertLessCircuit{}, &assertLessCircuit{A: NewU32(1234), B: NewU32(1233), Expected: frontend.Variable(0)}, ecc.BN254.ScalarField())
	assert.Error(err)

	err = test.IsSolved(&assertLessCircuit{}, &assertLessCircuit{A: NewU32(12), B: NewU32(123), Expected: frontend.Variable(1)}, ecc.BN254.ScalarField())
	assert.NoError(err)

	err = test.IsSolved(&assertLessCircuit{}, &assertLessCircuit{A: NewU32(2345), B: NewU32(3), Expected: frontend.Variable(0)}, ecc.BN254.ScalarField())
	assert.Error(err)

	err = test.IsSolved(&assertLessCircuit{}, &assertLessCircuit{A: NewU32(0), B: NewU32(0), Expected: frontend.Variable(0)}, ecc.BN254.ScalarField())
	assert.Error(err)
}
